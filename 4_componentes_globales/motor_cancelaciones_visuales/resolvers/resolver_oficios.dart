// lib/4_componentes_globales/motor_cancelaciones_visuales/resolvers/resolver_oficios.dart
...
class ResolverOficios implements ResolverCancelacion {
  @override
    ...
    switch (contexto.accion) {
      ...
      case TipoAccionCancelacion.avisoClienteCanceladoPorPro:
        return PanelAvisoCancelacionCliente(
          onRepublicar: contexto.onRepublicar, 
          onEntendido: contexto.onEntendido ?? () {},
        );
  ...
  Widget construirVistaInPlace(BuildContext context, CancelacionContexto contexto) {
    if (contexto.accion == TipoAccionCancelacion.vistaInPlace) {
      ...
      final soyCliente = contexto.actor == ActorCancelacion.cliente;
      
      // 🛡️ REFACTOR: Mapeo absoluto para saber de dónde provino la ruptura y si mutó de estado
      final bool fueCanceladoPorCliente = estado == 'cancelada_por_cliente' || estado == 'cancelada_vista_pro';
      final bool fueCanceladoPorPro = estado == 'cancelada_por_pro' || estado == 'cancelada_vista_cliente';

      // 🛡️ REFACTOR: Ahora evaluamos con la verdad transaccional
      final bool yoCancele = (soyCliente && fueCanceladoPorCliente) || (!soyCliente && fueCanceladoPorPro);

      return PanelEstadoCanceladoGlobal(
        titulo: yoCancele ? 'CANCELASTE ESTE TRABAJO' : 'TRABAJO CANCELADO',
        subtitulo: yoCancele 
            ? 'Has cancelado este trabajo. La contraparte ha sido notificada.' 
            : 'La contraparte ha cancelado este trabajo.',
        esError: true,
      );
    }
