...
class ModeloOficioTrabajo implements TrabajoContratable {
  ...
  factory ModeloOficioTrabajo.fromJson(Map<String, dynamic> json) {
    String desc = json['descripcion']?.toString() ?? '';
    ...
    if (desc.trim().startsWith('{')) {
      try {
        ...
        if (imagenes.isEmpty && map['images'] is List) imagenes = List<String>.from(map['images']);
      } catch (_) {}
    }
    ...
    final parsedPujas = (json['pujas'] as List<dynamic>? ??[]).map((e) => ModeloPuja.fromJson(e)).toList();
    bool cCalifico = json['cliente_califico'] == true || json['clienteCalifico'] == true;
    bool pCalifico = json['pro_califico'] == true || json['proCalifico'] == true;
    
    if (!cCalifico && parsedPujas.any((p) => p.clienteCalificoPuja == true)) {
      cCalifico = true;
    }
    if (!pCalifico && parsedPujas.any((p) => p.proCalificoPuja == true)) {
      pCalifico = true;
    }

    return ModeloOficioTrabajo(
      id: json['id']?.toString() ?? '',
      ...
      clienteCalifico: cCalifico,
      proCalifico: pCalifico,
