// lib/5_modulos/modulo_gestion_jornadas/servicios/servicio_actividad_jornadas_supabase.dart
import 'package:supabase_flutter/supabase_flutter.dart';
import '../../../3_modelos/modelo_jornada.dart';
import '../../../3_modelos/modelo_puja.dart';

class ServicioActividadJornadasSupabase {
  ...
  static Future<List<ModeloJornada>> obtenerPublicacionesClienteJornadas(String clienteId, Map<String, int> unreadCounts) async {
    ...
    return (res as List).map((row) {
      ...
      return ModeloJornada.fromJson(rowMap); 
    }).toList();
  }

  static Future<List<ModeloJornada>> obtenerPostulacionesProfesionalJornadas(String profesionalId, Map<String, int> unreadCounts) async {
    final res = await _client.from('pujas')
        .select('*, trabajos(*, perfiles!cliente_id(apodo, foto_url, rating, rating_cliente, cantidad_resenas, cantidad_resenas_cliente))')
        .eq('profesional_id', profesionalId).order('id', ascending: false);

    final List<ModeloJornada> trabajosPostulados =[];
    for (var row in (res as List)) {
      final rowMap = Map<String, dynamic>.from(row); 
      ...
      if (job['dificultad']?.toString() != 'jornada') continue;
      ...
      job['pro_califico'] = rowMap['pro_califico_puja'] == true;
      job['cliente_califico'] = rowMap['cliente_califico_puja'] == true;

      trabajosPostulados.add(ModeloJornada.fromJson(job)); 
    }
    return trabajosPostulados;
  }

  static Future<List<ModeloJornada>> obtenerSolicitudesDirectasProfesionalJornadas(String profesionalId, Map<String, int> unreadCounts) async {
    final res = await _client.from('trabajos')
