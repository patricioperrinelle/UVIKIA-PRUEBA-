// lib/5_modulos/modulo_gestion_jornadas/controladores/controlador_actividad_jornadas.dart
...
import '../servicios/servicio_actividad_jornadas_supabase.dart';
...
class ControladorActividadJornadas extends ChangeNotifier with WidgetsBindingObserver implements FuenteDeActividad {
  static final ControladorActividadJornadas _instancia = ControladorActividadJornadas._interno();
  factory ControladorActividadJornadas() => _instancia;
  
  ControladorActividadJornadas._interno() {
  ...
  @override
  DominioApp get dominio => DominioApp.jornadas;

  @override
  String get tituloActivoCliente => 'Jornadas publicadas';
  @override
  IconData get iconoActivoCliente => Icons.calendar_today_rounded;
  @override
  Color get colorTemaCliente => ColoresApp.secundarioCyan;
  @override
  String get textoVacioCliente => 'No tienes turnos o jornadas publicadas.';

  @override
  String get tituloActivoPro => 'Postulaciones a jornadas';
  @override
  IconData get iconoActivoPro => Icons.calendar_today_rounded;
  @override
  Color get colorTemaPro => ColoresApp.secundarioCyan;
  @override
  String get textoVacioPro => 'No te has postulado a ninguna jornada.';
  ...
  List<ModeloJornada> jornadasActivasCliente = [];
  List<ModeloJornada> historialFinalizadoCliente = [];
  List<ModeloJornada> historialCanceladoCliente = [];

  List<ModeloJornada> postulacionesJornadasActivasPro = [];
  List<ModeloJornada> historialFinalizadoPro = [];
  List<ModeloJornada> historialCanceladoPro = [];
  ...
  void _manejarEventoRealtime(dynamic payload) {
    ...
    if (tabla == 'trabajos') {
      ...
      if (oId == uid || pAsig == uid || pSol == uid) {
        ...
        if (tId != null) {
          final existeEnMisListasPro = postulacionesJornadasActivasPro.any((t) => t.id == tId);
      ...
      if (record['profesional_id']?.toString() == uid) {
        ...
        if (tId != null) {
          final existeEnMisListasCliente = jornadasActivasCliente.any((t) => t.id == tId);
    ...
    if (tipoEvento.contains('update') && payload.newRecord != null) {
      ...
      parcheAplicado = _parchearEnLista(jornadasActivasCliente, newRec) || parcheAplicado;
      parcheAplicado = _parchearEnLista(postulacionesJornadasActivasPro, newRec) || parcheAplicado;
  ...
  bool _parchearEnLista(List<ModeloJornada> lista, Map<String, dynamic> record) {
    ...
    if (index != -1) {
      ...
      lista[index] = ModeloJornada.fromJson(mapFusionado);
      return true;
    }
    return false;
  }
  ...
  bool _incrementarMensajeNoLeido(String trabajoId) {
    ...
    final listas = [jornadasActivasCliente, postulacionesJornadasActivasPro];
    ...
    for (var lista in listas) {
      ...
      if (index != -1) {
         ...
         lista[index] = ModeloJornada.fromJson(map);
         parcheado = true;
      }
    }
    return parcheado;
  ...
  Future<void> iniciarMotorReactivo() async {
    ...
    _trabajosChannel = Supabase.instance.client.channel('public:trabajos_jornadas_$uid');
    _trabajosChannel!
    ...
    _pujasChannel = Supabase.instance.client.channel('public:pujas_jornadas_$uid');
    _pujasChannel!
        .onPostgresChanges(event: PostgresChangeEvent.all, schema: 'public', table: 'pujas', callback: _manejarEventoRealtime)
        .subscribe(_manejarEstadoSuscripcion); 

    _mensajesChannel = Supabase.instance.client.channel('public:mensajes_jornadas_$uid');
    _mensajesChannel!
        .onPostgresChanges(event: PostgresChangeEvent.insert, schema: 'public', table: 'mensajes', filter: PostgresChangeFilter(type: PostgresChangeFilterType.eq, column: 'receptor_id', value: uid),
            callback: (payload) async {
              ...
              await ServicioActividadJornadasSupabase.marcarNotificacionBurbujaComoNoLeida(trabajoId, emisorId, uid);
  ...
  void eliminarRegistro(TrabajoContratable trabajo, bool esDueno) {
    if (trabajo is ModeloJornada) {
      _eliminarRegistroEspecifico(trabajo, esDueno);
    }
  }

  void _eliminarRegistroEspecifico(ModeloJornada trabajo, bool esDueno) {
      ...
      operacionRed: () async {
        if (esDueno) { 
          if (trabajo.tuvoContratoAlgunaVez) {
            await ServicioActividadJornadasSupabase.archivarTrabajoCancelado(trabajo.id);
          } else {
            await ServicioActividadJornadasSupabase.eliminarTrabajoFisicamente(trabajo.id);
          }
        } else {
          if (trabajo.profesionalSolicitadoId == Supabase.instance.client.auth.currentUser?.id && trabajo.pujaId == null) { 
            await ServicioActividadJornadasSupabase.rechazarSolicitudDirecta(trabajo.id); 
          } else if (trabajo.pujaId != null) { 
            await ServicioActividadJornadasSupabase.eliminarPujaVisualmenteParaPro(trabajo.pujaId!); 
          }
        }
      },
  ...
  void _verificarYDispararTrampaGlobal() {
    ...
    ModeloJornada? atrapado;
    final miId = Supabase.instance.client.auth.currentUser?.id;
    if (miId == null) return;

    final todosActivos = [
      ...jornadasActivasCliente, ...postulacionesJornadasActivasPro
    ];
  ...
  Future<void> recargarDatosDesdeCero() async {
    ...
    if (miId == null) return;
    iniciarMotorReactivo(); 
    if (jornadasActivasCliente.isEmpty && postulacionesJornadasActivasPro.isEmpty) await _cargarDesdeCacheLocal(miId); 
  ...
  void _removerLocalmente(String id) {
    jornadasActivasCliente.removeWhere((t) => t.id == id);
    postulacionesJornadasActivasPro.removeWhere((t) => t.id == id);
  ...
  DateTime _obtenerFechaActividadMasReciente(ModeloJornada trabajo) {
  ...
  void _ordenarCronologicamente(List<ModeloJornada> lista) {
    lista.sort((a, b) {
  ...
  void _ordenarTodasLasListas() {
    _ordenarCronologicamente(jornadasActivasCliente);
    _ordenarCronologicamente(historialFinalizadoCliente);
    _ordenarCronologicamente(historialCanceladoCliente);

    _ordenarCronologicamente(postulacionesJornadasActivasPro);
  ...
  Future<void> _cargarDesdeCacheLocal(String miId) async {
    try {
      ...
      final cacheStr = prefs.getString('actividad_jornadas_cache_$miId');
      if (cacheStr != null && cacheStr.isNotEmpty) {
        final data = jsonDecode(cacheStr);
        List<ModeloJornada> parseLista(String key) => (data[key] as List? ?? []).map((e) => ModeloJornada.fromJson(e)).toList();

        jornadasActivasCliente = parseLista('jornadasActivasCliente');
        ...
        postulacionesJornadasActivasPro = parseLista('postulacionesJornadasActivasPro');
  ...
  Future<void> _fetchSilenciosoBajoCapot(String miId) async {
    ...
    try {
      final unreadCounts = await ServicioActividadJornadasSupabase.obtenerMensajesNoLeidos(miId);
      
      final futures = await Future.wait([
        ServicioActividadJornadasSupabase.obtenerPublicacionesClienteJornadas(miId, unreadCounts),
        ServicioActividadJornadasSupabase.obtenerPostulacionesProfesionalJornadas(miId, unreadCounts),
        ServicioActividadJornadasSupabase.obtenerSolicitudesDirectasProfesionalJornadas(miId, unreadCounts), 
      ]);
      ...
      // --- FILTROS CLIENTE ---
      final jornadasCliente = misPublicaciones;

      historialFinalizadoCliente = jornadasCliente.where((i) => i.estado == 'finalizado' && i.clienteCalifico == true).toList();
      
      historialCanceladoCliente = jornadasCliente.where((i) => 
          i.estado == 'cancelado' && 
          i.tuvoContratoAlgunaVez && 
          i.estadoNegociacion != 'cancelada_por_pro' 
      ).toList();

      final activasYCanceladasSinContratoCliente = jornadasCliente.where((i) =>
        (i.estado != 'finalizado' && i.estado != 'cancelado' && i.estado != 'checkout_bloqueado_temporal'
            && i.estado != 'pendiente_pago' && i.estado != 'expirada') ||
        (i.estado == 'cancelado' && !i.tuvoContratoAlgunaVez) ||
        (i.estado == 'finalizado' && i.clienteCalifico == false) ||
        (i.estado == 'cancelado' && i.estadoNegociacion == 'cancelada_por_pro')
      ).toList();

      jornadasActivasCliente = activasYCanceladasSinContratoCliente.where((i) => i.profesionalSolicitadoId == null).toList();
      
      // --- FILTROS PROFESIONAL ---
      final Map<String, ModeloJornada> mapaDeduplicado = {};
      ...
      String _getMiEstadoPuja(ModeloJornada t) { return t.estadoNegociacion ?? ''; }
      ...
      postulacionesJornadasActivasPro = activasYRechazadasPro.where((i) => i.profesionalSolicitadoId == null).toList();
      ...
      debugPrint('[SQL] Error fetch silencioso en Historial Jornadas: $e');
    } finally {
      if (isLoading) {
        isLoading = false;
      }
  ...
  Future<void> _guardarCacheLocal(String miId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final data = {
        'jornadasActivasCliente': jornadasActivasCliente.map((e) => e.toJson()).toList(),
        ...
        'postulacionesJornadasActivasPro': postulacionesJornadasActivasPro.map((e) => e.toJson()).toList(),
      ...
      await prefs.setString('actividad_jornadas_cache_$miId', jsonEncode(data));
    } catch (_) {}
  }

  void _calcularBadgesTotales() {
    int cliente = 0;
    for (var i in jornadasActivasCliente) { cliente += calcularAlertasItem(i, true); }
    alertasTabCliente = cliente;

    int pro = 0;
    for (var i in postulacionesJornadasActivasPro) { pro += calcularAlertasItem(i, false); }
    alertasTabPro = pro;
  }
  ...
  int _calcularAlertasItemEspecifico(ModeloJornada item, bool esDueno) {
  ...
  void _marcarItemComoVistoEspecifico(ModeloJornada item, bool esDueno) async {
  ...
  @override
  List<TrabajoContratable> obtenerActivosCliente() => jornadasActivasCliente;
  ...
  @override
  List<TrabajoContratable> obtenerActivosPro() => postulacionesJornadasActivasPro;
