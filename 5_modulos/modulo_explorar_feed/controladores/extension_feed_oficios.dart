// lib/5_modulos/modulo_explorar_feed/controladores/extension_feed_oficios.dart
part of 'controlador_feed_oficios.dart';

extension ExtensionOficios on ControladorFeedOficios {
  
  Future<void> _fetchOficiosSiguientePagina() async {
    final genActual = _generacionOficios;
    try {
      final ciudadBusqueda = provinciaFiltro;
      final localidadBusqueda = localidadFiltro;
      
      List<Map<String, dynamic>> rawJobs = await ServicioFeedSupabase.obtenerOficiosPaginadosV5(
        ciudad: ciudadBusqueda, 
        localidad: localidadBusqueda,
        misOficios: misOficiosRealesCache, 
        categoria: categoriaFiltro,
        keyword: palabraClave,
        cursorEsOficio: _motorOficios.cursor.esOficio,
        cursorEsCiudad: _motorOficios.cursor.esCiudad,
        cursorEsLocalidad: _motorOficios.cursor.esLocalidad,
        cursorRank: _motorOficios.cursor.rank,
        cursorRotacion: _motorOficios.cursor.rotacion,
        cursorFecha: _motorOficios.cursor.fecha, 
        cursorId: _motorOficios.cursor.id,
        limit: 20, 
      );
      debugPrint("ALPHA-CORE DEBUG: Se descargaron ${rawJobs.length} oficios de Supabase.");

      if (genActual != _generacionOficios) return;

      if (_isRefreshOficios) {
        _motorOficios.elementos.clear(); 
        _isRefreshOficios = false;
      }
      ...
      if (rawJobs.isNotEmpty) {
        ...
        final catLast = last['categoria']?.toString() ?? '';
        
        _motorOficios.cursor.esOficio = misOficiosRealesCache.split(',').contains(catLast);
        _motorOficios.cursor.esCiudad = ciudadBusqueda.isNotEmpty && (last['ciudad'] == ciudadBusqueda);
        _motorOficios.cursor.esLocalidad = localidadBusqueda.isNotEmpty && (last['localidad'] == localidadBusqueda);
        
        _motorOficios.cursor.rank = (last['rank_calc'] as num?)?.toDouble() ?? 0.0;
        _motorOficios.cursor.rotacion = (last['rotacion_calc'] as num?)?.toInt() ?? 2147483647;
        _motorOficios.cursor.fecha = last['fecha_hora'] ?? '2000-01-01T00:00:00Z'; 
        _motorOficios.cursor.id = last['id'] ?? '00000000-0000-0000-0000-000000000000';
        
        final nuevos = await compute(_mapearListaOficiosBackground, rawJobs);
        
        _motorOficios.anexarPaginaSegura(nuevos, esRefresh: false);
        
      } else {
        _motorOficios.hasReachedMax = true;
        _motorOficios.anexarPaginaSegura([], esRefresh: false);
      }
    } catch (e) { debugPrint('Error Oficios V5.7: $e'); }
  }

  Future<List<ModeloOficioTrabajo>> obtenerPagina1OficiosSilenciosa() async {
    try {
      ...
      List<Map<String, dynamic>> rawJobs = await ServicioFeedSupabase.obtenerOficiosPaginadosV5(
        ...
        categoria: categoriaFiltro, 
        keyword: palabraClave,      
        cursorEsOficio: cursorLimpio.esOficio,
      ...
      return await compute(_mapearListaOficiosBackground, rawJobs);
    } catch (e) {
      debugPrint('SWR-Titan: Error Extracción SWR Silenciosa (Oficios): $e');
      return [];
    }
  }
}

List<ModeloOficioTrabajo> _mapearListaOficiosBackground(List<Map<String, dynamic>> rawList) {
  return rawList.map((row) => _mapearOficioUnico(row)).toList();
}

ModeloOficioTrabajo _mapearOficioUnico(Map<String, dynamic> row) {
  ...
  return ModeloOficioTrabajo.fromJson(jobMap);
}
