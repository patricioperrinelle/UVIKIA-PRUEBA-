// lib/5_modulos/modulo_explorar_feed/pantallas/pantalla_feed_jornadas.dart
...
import 'pantalla_favoritos_jornadas.dart'; 

class PantallaFeedJornadas extends StatefulWidget {
  const PantallaFeedJornadas({Key? key}) : super(key: key);

  @override
  State<PantallaFeedJornadas> createState() => _PantallaFeedJornadasState();
}

class _PantallaFeedJornadasState extends State<PantallaFeedJornadas> {
  ...
  void _abrirJornadaONotificar(ModeloJornada jornadaTipada, bool isDueno, bool esModoCliente) {
    if (isDueno || !esModoCliente) {
      Navigator.push(
        ...
        if (resultado == true) {
          _controlador.eliminarTrabajoDeRAM(jornadaTipada.id);
        } else {
  ...
  void _abrirFavoritosCongelandoFeed() {
    GestorSesionGlobal.requerirAuth(() {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => PantallaFavoritosJornadas(controlador: _controlador),
        ),
      );
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(esModoCliente ? 'Muro de Jornadas' : 'Empleos y Turnos', 
          style: TextStyle(fontWeight: FontWeight.bold, color: tema.colorScheme.onSurface, fontSize: 18)), 
        leading: IconButton(
          ...
          children:[
              ...
              Container(
                ...
                child: Row(
                  children:[
                    ...
                    const SizedBox(width: 8),
                    Expanded(child: Text('Modo Cliente: Cambia a tu perfil PRO para postularte.', 
            ...
            ListenableBuilder(
              ...
              builder: (context, child) {
                final lista = _controlador.jornadas; 
                          ...
                          child: _controlador.isLoading && lista.isEmpty
                              ...
                              : lista.isEmpty
                                  ? EstadoVacioFeed(
                                      ...
                                      titulo: 'No hay turnos que coincidan', 
                                      ...
                                      children: [
                                        Expanded(
                                          child: GridView.builder(
                                            controller: _controlador.scrollJornadas, 
                                            ...
                                            itemBuilder: (context, index) {
                                              final jornada = lista[index];
                                              final bool yaOfertado = _controlador.misPostulacionesIds.contains(jornada.id);
                                              final bool isDueno = jornada.ownerId == miId;
                                              final bool esGuardado = _controlador.misTrabajosGuardadosIds.contains(jornada.id);
                                              ...
                                              return TarjetaJornadaFeed(
                                                ...
                                                onTapGuardar: () => _controlador.toggleTrabajoGuardadoGlobal(context, jornada, true),
                                                onTapTipado: (jornadaTipada) => _abrirJornadaONotificar(jornadaTipada as ModeloJornada, isDueno, esModoCliente),
                                              );
                                            },
                                          ),
                                        ),
                                        if (_controlador.isLoadingMoreJornadas)
                                          const Padding(
